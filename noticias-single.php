...
$SqlQuery_tbnoticia = "SELECT * FROM tbnoticia WHERE idtbnoticia = '$id_noticia'";
$result_tbnoticia = $Conn_tbnoticia->connect()->query($SqlQuery_tbnoticia);
$rows_tbnoticia = $result_tbnoticia->fetch_assoc();
$datatbnoticia = new DateTime($rows_tbnoticia['datetbnoticia']);
switch($datatbnoticia->format('m')){
	case "01": $mesnoticia = "Janeiro"; break;
	case "02": $mesnoticia = "Fevereiro"; break;
	case "03": $mesnoticia = "Março"; break;
	case "04": $mesnoticia = "Abril"; break;
	case "05": $mesnoticia = "Maio"; break;
	case "06": $mesnoticia = "Junho"; break;
	case "07": $mesnoticia = "Julho"; break;
	case "08": $mesnoticia = "Agosto"; break;
	case "09": $mesnoticia = "Setembro"; break;
	case "10": $mesnoticia = "Outubro"; break;
	case "11": $mesnoticia = "Novembro"; break;
	case "12": $mesnoticia = "Dezembro"; break;
}
?>
<!DOCTYPE html>
<html lang="en-US">
<head>
	<meta charset="UTF-8"/>
	<title>Notícia | Renovação Carismática Católica &amp; Diocese Divinópolis</title>
...
<div class="boxed-container">
	...
	<div class="main-title" style="background-color: #f2f2f2; ">
		<div class="container">
			<h1 class="main-title__primary"><?php echo $rows_tbnoticia['titletbnoticia'];?></h1>
			<h3 class="main-title__secondary">
				<i class="fa fa-calendar" style="margin-right:10px; margin-top:15px;"></i>
				<time datetime="2014-10-20T09:36:46+00:00" class="meta-data__date"><?php echo $datatbnoticia->format('d')." de ".$mesnoticia." de ".$datatbnoticia->format('Y');?></time>	
			</h3>
		</div>
	</div>
	<div class="breadcrumbs breadcrumbs--page-builder">
		<div class="container">
			...
			<span typeof="v:Breadcrumb"><a rel="v:url" property="v:title" title="noticias" href="noticias.php">noticias</a></span>
			<?php
				$len =strlen($rows_tbnoticia['titletbnoticia']);
				if ($len>50) {
                	$titulo_noticia=substr($rows_tbnoticia['titletbnoticia'],0,51).'...';
                }
				else{
					$titulo_noticia=$rows_tbnoticia['titletbnoticia'];
				}
			?>
			<span typeof="v:Breadcrumb"><span property="v:title"><?php echo $titulo_noticia;?></span></span>			
		</div>
	</div>
			...
			<div class="row">
				<main class="col-xs-12  col-md-12" role="main">
					<article class="post tformat-standard hentry">
						<a href="<?php echo 'data:image/png;base64,' . base64_encode($rows_tbnoticia['imagetbnoticia'])  ; ?>">
							<?php echo '<img src="data:image/png;base64,' . base64_encode( $rows_tbnoticia['imagetbnoticia'] ) . '" alt='.$rows_tbnoticia['titletbnoticia'].' class="img-responsive wp-post-image" width="848" height="480" />' ?>
						</a>
						<div class="meta-data">
							<i class="fa fa-calendar" style="margin-right:10px; margin-top:15px;"></i>
							<time datetime="2014-10-20T09:36:46+00:00" class="meta-data__date"><?php echo $datatbnoticia->format('d')." de ".$mesnoticia." de ".$datatbnoticia->format('Y');?></time>							
						...
						<div class="hentry__content" style="text-align: justify;">
							<p>
								...
								$SqlQuery_tbparagraphnotice = "SELECT * FROM tbparagraphnotice WHERE idtbnoticia = '$id_noticia'";
								$result_tbparagraphnotice = $Conn_tbparagraphnotice->connect()->query($SqlQuery_tbparagraphnotice);
								while($rows_tbparagraphnotice = $result_tbparagraphnotice->fetch_assoc()){
									echo $rows_tbparagraphnotice['paragraphtbparagraphnotice'];
								}
								?>
							</p>
						</div>
						<div class="meta-data">														
							<span class="meta-data__comments"><a href="<?php echo $rows_tbnoticia['fontetbnoticia'];?>" target="_blank"><?php echo "Fonte: ".$rows_tbnoticia['fontetbnoticia'];?></a></span>
						</div>
